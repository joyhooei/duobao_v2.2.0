...
function recordGetDataType(recordType) {
	...
	dropRefresh('#p-car-record',recordRefresh);
...
function recordGetData(flag,ele,curPage,callback){
	ele = '#all-lottery-content'
	$.ajax({
		type:"post",
		url:luanmingli.getUrl+"getPersonResult",
		data:{
				...
				'<div class="list-block media-list">'+
					'<ul>'+
						'<li class="item-content clearfix">'+
							...
							'<div style="padding-top:1rem;" class="item-inner clearfix">'+
...
function recordBottomLoadmore(){
	$(document).off('infinite', '#p-car-record .infinite-scroll');
//	console.log(curPage)
	var loadMoreFlag = false;
	
	$(document).on('infinite', '#p-car-record .infinite-scroll',function() {
		if (loadMoreFlag) {
			return;
		}
